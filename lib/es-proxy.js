...
exports.configureESProxy = function(app, esHost, esPort, esUser, esPassword, basePath) {
  app.use(basePath + "/__es", function(request, response, next) {
    ...
    if (request.method == 'PUT' || request.method == 'POST') {
      ...
      // deny if there is no group for the authenticated user or the group does not match the start of the dashboard name
      if (!response.grafana_editor_group || dashboard.toUpperCase().indexOf(response.grafana_editor_group.toUpperCase()) != 0) {
        response.end();
        return request.url;
      } else {
        console.log("dashboard prefix match for " + response.grafana_editor_group + ", authorising");
      }
    }
